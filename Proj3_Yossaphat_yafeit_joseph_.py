...
height = 250 # int(ppmm*50) # 50mm box requirement
...
def get_valid_coord(label):
    while True:
        ...
        try:
            ...
            py = y #height - y # convert Cartesian y → pygame y (inverted)
...
def getPossibleMoves(new_node_x, row, col, counter, L):
    ...
    for neighbor, move_cost in neighbors:
        ...
        if neighbor not in node_to_g_score or tentative_g < node_to_g_score[neighbor]:
            ...
            heapq.heappush(open_heap, ((node_to_f_score[neighbor]), tentative_g, neighbor)) # saving to heap
...
open_heap = []
heapq.heappush(open_heap, (initial_dist, 0.0, goal_state))
...
while node_to_f_score:
    ...
    f_cost, g_cost, current_node = heapq.heappop(open_heap) # pops lowest. Heaps became prefferable over ques for speed and ease of use (automatically pops min value)
    ...
    del node_to_f_score[current_node] # [0]]  

    # print("\nis goal reached test: ", is_goal_reached(current_node, start_state))
    ...
    getPossibleMoves(current_node, a, b, counter, L)
...
for i in range(len(shortest_path)):
    # print(state)
    pygame.draw.circle(surface, color=(255, 0, 255), center=(shortest_path[i][0], shortest_path[i][1]), radius=2)
